...
| `--report` | `report` | `False` | Generate a professional PDF lab report. |
| `--json` | `json` | `False` | Generate a machine-readable JSON report file. |
| `--render` | `render` | `False` | Render a GIF animation of the simulation. |
| `--cut` | `cut` | `False` | Apply a clipping plane (cross-section) during rendering. |
| `--hts` | `hts` | `None` | Path to a CSV file for High-Throughput Screening. |
| `--rank-by` | `rank_by` | `efficiency` | HTS ranking priority. Options: `rg` (smallest first), `efficiency` (highest first). |
| `--solvent` | `solvent` | `custom` | Predefined solvent profiles. Options: `water`, `ethanol`, `dmso`, `custom`. Sets `temp` and `damp` automatically. |

## Physics & Environment
| Flag | Variable | Default | Description |
| :--- | :--- | :--- | :--- |
| `--temp` | `temp` | `300.0` | **Temperature (K)**. Controls thermal energy. If `--solvent` is used, this is set automatically. |
| `--damp` | `damp` | `20.0` | **Damping (fs)**. Langevin thermostat parameter. Controls viscosity. |
